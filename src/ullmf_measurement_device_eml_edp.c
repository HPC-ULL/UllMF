...
#define MEASUREMENT_TIME_INTERVAL 150 // 150 ms
#define INTERNAL_REFRESH_INTERVAL 5


static enum ullmf_measurement_error get_eml_edp_measurements(measurement_device_eml_edp_t * md_eml) {
    emlData_t* data[md_eml->ndevices];
    md_eml->err = emlStop(data);
    md_eml->time = MPI_Wtime() - md_eml->time;
    ...
    md_eml->parent.measurement *= md_eml->time;
...
static enum ullmf_measurement_error init(void* self) {
    ...
    measurement_device_eml_edp_t * self_md_eml = (measurement_device_eml_edp_t *) self;
...
static enum ullmf_measurement_error shutdown(void* self) {
    ...
    if (self_md_eml->parent.measuring) {
        self_md_eml->err = get_eml_edp_measurements(self_md_eml);
        if (self_md_eml->err != EML_SUCCESS)
            return ULLMF_MEASUREMENT_INTERNAL_LIBRARY_ERROR;
    }
    ...
    if (self_md_eml->ndevices > 0) {
        ...
        if (self_md_eml->measurement_interval > 0 &&
            ...
            if (!self_md_eml->interval_calc_started) {
            	...
            	self_md_eml->first_calibration_t = MPI_Wtime();
            }
        }
    } else {
    	if (self_md_eml->ndevices == 0) {
...
static enum ullmf_measurement_error calculate_measurement_interval(measurement_device_eml_edp_t* self) {
	self->first_calibration_t = MPI_Wtime() - self->first_calibration_t;
	double iteration_time = (double) self->first_calibration_t / self->internal_calibration_interval;
...
static enum ullmf_measurement_error measurement_stop(void* self) {
       ...
       )
    {
    	...
    	if (self_md_eml->current_it == self_md_eml->internal_calibration_interval - 1) {
    	    dbglog_info("Calculating measurement interval\n");
...
measurement_device_eml_edp_t ullmf_eml_edp_device = {
